// Aura Cosmos backend
// - Proxies Dubis chat requests to Anthropic (API key never touches the browser)
...
/* ---------------------------------------------------------
   Simple in-memory per-IP rate limiters (fine for a small app;
   swap for a Redis-backed limiter for real production traffic).
---------------------------------------------------------- */
...
const dubisLimiter = createRateLimiter(30, 60 * 1000);   // 30/min
const authLimiter = createRateLimiter(8, 60 * 1000);     // 8/min — brute-force guard
const progressLimiter = createRateLimiter(60, 60 * 1000); // 60/min
...
function isValidUsername(v) { return typeof v === 'string' && USERNAME_RE.test(v); }
...
app.post('/api/auth/signup', async (req, res) => {
  ...
  try {
    ...
    const user = db.createUser({ id, username, email, passwordHash });
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    // so we don't leak which emails/usernames are registered.
...
/* ===========================================================
   CLOUD PROGRESS ROUTES (require auth)
=========================================================== */
...
app.put('/api/progress', auth.requireAuth, auth.requireXhrHeader, (req, res) => {
  if (progressLimiter(clientIp(req))) {
    return res.status(429).json({ error: 'Too many requests — please slow down a little.' });
  }
  const body = req.body;
...
/* ===========================================================
   DUBIS (Anthropic proxy) — unchanged from before
=========================================================== */
