...
function defaultProgress() {
  return {
    ...
    forgedStars: [],
    gameLevels: {},
    orbitRushHighScores: {desktop:0, mobile:0},
    dailyMystery: null,
    updatedAt: null
  };
}
...
function safeHighScores(v) {
  const out = { desktop: 0, mobile: 0 };
  if (v && typeof v === 'object') {
    out.desktop = safeNumber(v.desktop, 0, 0, 1_000_000);
    out.mobile = safeNumber(v.mobile, 0, 0, 1_000_000);
  }
  return out;
}
function safeDailyMystery(v) {
  if (!v || typeof v !== 'object') return null;
  if (typeof v.date !== 'string' || typeof v.hypothesis !== 'string' || typeof v.feedback !== 'string') return null;
  return {
    date: v.date.slice(0, 64),
    hypothesis: v.hypothesis.slice(0, 2000),
    feedback: v.feedback.slice(0, 4000),
    answer: typeof v.answer === 'string' ? v.answer.slice(0, 4000) : '',
    xp: safeNumber(v.xp, 0, 0, 1000),
    relevance: safeNumber(v.relevance, 0, 0, 10)
  };
}
